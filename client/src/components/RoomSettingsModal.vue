...
function leave() {
  // Просто покидаем комнату: родительская страница должна скрыть модалку по `room.currentRoom`.
  room.leaveRoom();
}
...
<template>
  <div class="modal-overlay">
    <div class="room-modal panel-cs">
      <header class="header">
        <div class="header-orange"></div>
        <h1 class="title">{{ room.currentRoom?.name }}</h1>
        <div class="header-actions">
          <span class="map-badge">
            {{ room.currentRoom?.map }} · {{ room.currentRoom?.maxPlayers }} слотов · до {{ room.currentRoom?.roundsToWin }} побед
          </span>
